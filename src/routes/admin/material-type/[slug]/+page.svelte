<script lang="ts">
	...
	const materialDetail = $derived(data.materialType);
		...
		onChange(event) {
			if (event) {
				if (event.paths.includes("name")) {
					if ($formData.name !== materialDetail.name) {
						changes = true;
					} else {
						changes = false;
					}
	...
	$effect(() => {
		$formData.name = materialDetail.name;
	});
	...
	$effect(() => {
		if (form?.delete) {
			...
			if (form.edit.success) {
				toast.success(`Material type ${form.edit.data?.name} edited successfully`);
				invalidateAll();
...
<div class="space-y-2 pr-2 pb-2">
	<h1 class="font-display text-3xl font-semibold tracking-tight">Material Type Detail</h1>
	<Card.Root id="edit">
		<Card.Header>
			<Card.Title class="font-display">Edit Material Type</Card.Title>
		</Card.Header>
		<form
			method="POST"
			...
			<Card.Content class="space-y-2">
				<Form.Field form={superform} name="name">
					...
					{#if $errors.name}
						<Form.FieldErrors />
					{:else}
						<Form.Description>Name of the material type to be displayed.</Form.Description>
					{/if}
				</Form.Field>
			</Card.Content>
	...
	<AlertDialog.Root>
		...
		<AlertDialog.Content>
			<form class="contents" action="?/delete" method="POST" use:svelteEnhance>
				<AlertDialog.Header>
					<AlertDialog.Title
						>Do you want to delete material type {materialDetail.name}?</AlertDialog.Title
					>
					<AlertDialog.Description>
						<input type="hidden" name="id" value={materialDetail.id} />
						This action is irreversible. Are you sure you want to delete
						<b>{materialDetail.name}</b>?
					</AlertDialog.Description>
				</AlertDialog.Header>
				<AlertDialog.Footer>
