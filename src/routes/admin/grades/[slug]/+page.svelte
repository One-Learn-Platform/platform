<script lang="ts">
	...
	const superform = superForm(data.form, {
		...
		onChange(event) {
			if (event) {
				if (event.paths.includes("level")) {
					if ($formData.level !== roleDetail.level) {
						changes = true;
					} else {
						changes = false;
					}
	...
	$effect(() => {
		$formData.level = roleDetail.level;
	});
...
<div class="space-y-2 pr-2 pb-2">
	<h1 class="font-display text-3xl font-semibold tracking-tight">Role Detail</h1>
	<Card.Root id="edit">
		<Card.Header>
			...
			<Card.Content class="space-y-2">
				<Form.Field form={superform} name="level">
					...
					{#if $errors.level}
						<Form.FieldErrors />
					{:else}
						<Form.Description>Name of the grade to be displayed.</Form.Description>
					{/if}
				</Form.Field>
			</Card.Content>
	...
	<AlertDialog.Root>
		...
		<AlertDialog.Content>
			<form class="contents" action="?/delete" method="POST" use:svelteEnhance>
				<AlertDialog.Header>
					<AlertDialog.Title>Do you want to delete grade {roleDetail.level}?</AlertDialog.Title>
					<AlertDialog.Description>
						<input type="hidden" name="id" value={roleDetail.id} />
						This action is irreversible. Are you sure you want to delete
						<b>{roleDetail.level}</b>?
					</AlertDialog.Description>
				</AlertDialog.Header>
				<AlertDialog.Footer>
