...
export const actions: Actions = {
	signin: async (event) => {
		...
		if (!success) {
			...
			return fail(403, {
				login: {
					success: false,
					data: null,
					message: "CAPTCHA Failed! Please refresh the page",
				},
				form,
			});
		}
		...
		if (!form.valid) {
			...
			return fail(400, {
				login: { success: false, data: null, message: "Content is invalid, please try again" },
				form,
			});
		}
		...
		if (existingUser.roleId === 1 || existingUser.roleId === 2) return redirect(302, "/admin");
