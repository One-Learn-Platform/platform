...
import { classroom, grades, report, user } from "$lib/schema/db";
	...
	if (event.locals.user) {
		...
		const leaderboard = await db
			.select({
				...
				score: avg(report.score),
				avatar: user.avatar,
				gradeLevel: grades.level,
			})
			.from(report)
			.innerJoin(user, eq(report.userId, user.id))
			.innerJoin(classroom, eq(report.classroomId, classroom.id))
			...
			.where(eq(report.schoolId, schoolId))
			.orderBy(desc(avg(report.score)));
